...
var getUnborrowedBooks = (id,callback) => {
    var sql = "SELECT * FROM books WHERE (user_id = '"+id+"') OR (user_id = 0)";
    db.executeQuery(sql, null, function(result) {
        callback(result);
    });
};
...
var customerSearch = (searchBy, word,id, callback) => {
    var sql = "SELECT * FROM books WHERE "+searchBy+" = ? AND (user_id = "+id+" OR user_id = 0)";
    db.executeQuery(sql, [word], function(result) {
        callback(result);
    });
};
